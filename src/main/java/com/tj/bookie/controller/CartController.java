...
public class CartController {
    ...
    @GetMapping(path="/get")
    public ResponseEntity<?> get(@RequestParam @NotEmpty String wx_id,
                                 @RequestParam @Range(min = 0) Integer page) throws JSONException {
        return cartService.get(wx_id, page);
    }


    @ApiOperation(value="从购物车删除", notes = "返回1或0，1表示成功，0多半是购物车没有那一项，用户不存在会404")
    @DeleteMapping(path="/delete")
    public ResponseEntity<?> delete(@RequestParam @NotEmpty String wx_id,
                                 @RequestParam @Range(min = 0) Integer bookId) {
        return cartService.delete(wx_id, bookId);
    }
}
